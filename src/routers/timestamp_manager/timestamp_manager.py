...
@timestamp_router.get("/delete-all-timestamp", description="eliminate all the timestamp record for all addresses")
def delete_all_timestamp(timeManager:Annotated[TimestampManager, FromDishka()]):
    return timeManager.delete_all_last_fetch()
    


@timestamp_router.get("/delete-address-timestamp", description="eliminate the timestamp record for one address")
